...
func generateChoiceOutput():
	for arrChoice in range(1,maxChatLine + 1):
		get_node("Choice0" + str(arrChoice - 1)).visible = true
		get_node("Choice0" + str(arrChoice - 1) + "/ChatText").text =  LoaderText.getChatText(chatGlobalCode,chatNo,arrChoice)
		choiceChatJump[arrChoice - 1] =  int(LoaderText.getChatJump(chatGlobalCode,chatNo,arrChoice))
...
#	send value array choiceChatJump, based on button click,  int(obj[-1]) -> right 1 of choiceBtn Name Clicked-> noChoiceClicked
	chatJump = choiceChatJump[choiceAnswered]
	calculateJumpPoint()
	nextProcess()
